...
├── resources.html              # GitHub-OAuth-gated internal resources (lab manual, CMS link, profile edit)
├── TEAM_EDITING_GUIDE.md       # User-facing guide for editing team data via files (not built into site)
├── assets/
│   ├── css/style.css           # Single stylesheet (~1410 lines)
│   └── js/main.js              # Auth (GitHub OAuth), PubMed widget + caching, member-pub matching, spotlight lightbox, mobile nav, alumni collapse (~540 lines)
...
1. Edit `_data/team.yml`. The categories rendered on `team.html` are: `staff`, `postdocs`, `trainees`, `alumni` (alumni shown in a collapsible section). A `collaborators` key also exists in the YAML but is currently not rendered on the team page. Include a `slug` field to link to a profile page:
```yaml
staff:
  - name: "New Person, MS"
    role: "Data Scientist"
...
- `resources.html` is GitHub-OAuth-gated: only visible to members of the `Healthcare-Allocation-Lab` org. The auth flow is the same Cloudflare Worker proxy used by the CMS; gating logic lives in `assets/js/main.js`
- `TEAM_EDITING_GUIDE.md` is a user-facing guide for editing team data by hand (not rendered on the site). Keep it in sync if conventions change for `_data/team.yml` or `_team_members/`
